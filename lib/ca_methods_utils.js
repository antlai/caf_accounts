...
exports.sendPurchaseConfirm = function(self, email, props) {
    const body = self.$.smtp.instantiateTemplate('purchase', props);
    const subject = `[Caf.js] Order Confirmation: ${props.id}`;
    return self.$.smtp.dirtySend(email, subject, body);
};

exports.sendMockPurchaseConfirm = function(self, email, props) {
    const body = self.$.smtp.instantiateTemplate('mockPurchase', props);
    const subject = `[Caf.js] Test Order Confirmation: ${props.id}`;
    return self.$.smtp.dirtySend(email, subject, body);
};
    ...
    if (!self.$.props.disableTestAccounts &&
        ...
        self.$.props.testAccounts
            .forEach(function(acc) {
                ...
                const email = `${acc.username}@localtest.me`;
