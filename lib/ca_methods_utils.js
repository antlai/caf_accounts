...
exports.checkUsername = function(self, account) {
    if (!tokens.validUsername(account)) {
        throw new Error('Invalid name');
    }
    var me = self.__ca_getName__();
    if (!tokens.validExtendedNobody(me)) {
        throw new Error('Destination CA is not an extended NOBODY user');
    }
    var caLocalName = json_rpc.splitName(me)[1];
    if (caLocalName !== account.substring(0, 2)) {
